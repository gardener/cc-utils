...
# stolen from https://github.com/karlkfi/concourse-dcind/blob/master/entrypoint.sh
# adapted for a slightly different use-case (we need more control, and only want to
# run dockerd if required, within a script):
# - do not kill dockerd after script ends
# - also, ignore ARGV, if present (early-exit before that)
