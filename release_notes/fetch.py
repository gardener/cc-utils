...
import release_notes.association as rna
import release_notes.models as rnm
...
def create_note_blocks(release_notes: set[rnm.ReleaseNote]) -> str:
...
def fetch_release_notes(
    ...
    commit_pulls = rna.request_pulls_from_api(
    ...
    for filter_out_commit in filter_out_commits:
        blacklisted_source_blocks.update(rnm.list_source_blocks(filter_out_commit.message))
        for pr in commit_pulls[filter_out_commit.hexsha]:
            blacklisted_source_blocks.update(rnm.list_source_blocks(pr.body))
    ...
    for filter_in_commit in filter_in_commits:
        ...
        for pr in commit_pulls[filter_in_commit.hexsha]:
            ...
            ) for z in rnm.list_source_blocks(pr.body))
        # by commit
        release_notes.update(rnm.create_release_note_obj(
            block=z,
            source_commit=filter_in_commit,
            ...
            source_component=component,
            current_component=component,
        ) for z in rnm.list_source_blocks(filter_in_commit.message))
