...
def update_images_data(images_data: Dict, new_versions_by_name: Dict[str, Dict]) -> List[Update]:
    ...
    for name, image_list in images_by_name.items():
        ...
        if not has_target_version:
            ...
            if all_newer_tags:
                ...
                updates.append({'image_name': name, 'old_tag': old_tag, 'new_tag': latest_tag, 'update_type': 'singleton'})
        else:
            ...
            for patch_tag in newer['patch']:
                ...
                for img in image_list:
                    ...
                    if img_version and (img_version[0], img_version[1]) == (major, minor):
                        old_tag = img[1]['tag']
                        img[1]['tag'] = patch_tag
                        updates.append({'image_name': name, 'old_tag': old_tag, 'new_tag': patch_tag, 'update_type': 'patch'})
                        break
            ...
            for minor_tag in newer['minor']:
                ...
                template_image['targetVersion'] = f"{parse_version(minor_tag)[0]}.{parse_version(minor_tag)[1]}.x"
                new_image_entries.append(template_image)
                
                # Find the previous highest version to set as 'old_tag'
                all_tags = [img['tag'] for _, img in image_list] + [t['tag'] for t in new_image_entries]
                ...
                updates.append({'image_name': name, 'old_tag': old_tag, 'new_tag': minor_tag, 'update_type': 'minor'})
            ...
            if all_image_entries_for_name:
                ...
                highest_entry = max(all_image_entries_for_name, key=lambda img: parse_version(img['tag']))
    ...
    if indices_to_remove:
        images_data['images'] = [img for i, img in enumerate(images_data['images']) if i not in indices_to_remove]
    
    images_data['images'].extend(new_image_entries)
    return updates

def find_intermediate_versions(current_tag: str, new_tag: str, available_tags: List[str]) -> List[str]:
    """
            ...
            if 'labels' in image:
                ...
                for label in image['labels']:
                    ...
                    for key, value in label['value'].items():
                        formatted_value = str(value).lower() if isinstance(value, bool) else f"'{value}'"
                        f.write(f"      {key}: {formatted_value}\n")
            
            if i < len(data['images']) - 1:
                f.write("\n")

def create_release_notes(updates: List[Update], images_data: Dict, all_available_tags: Dict[str, List[str]], filename: str):
    """
    ...
    repos_by_name = {img['name']: img['sourceRepository'] for img in images_data['images'] if 'sourceRepository' in img}
    ...
    for update in updates:
        ...
        if old_tag:
            intermediate = find_intermediate_versions(old_tag, new_tag, all_available_tags[image_name])
    ...
    with open(filename, 'w') as f:
        ...
        f.write("The following images have been updated. Please review the release notes for each component to check if changes need to be made to our Helm charts:\n\n")
        f.write("**Note**: All intermediate versions between the current and new version are listed to ensure no breaking changes are missed.\n\n")
...
def main():
    ...
    parser.add_argument('--images-yaml-path', default='imagevector/images.yaml', help='Path to the images.yaml file.')
    parser.add_argument('--release-notes-path', default='release-notes.md', help='Path where the release-notes.md file will be generated.')
    ...
    if not check_regctl_available():
        ...
        print("Please install regctl from https://github.com/regclient/regclient", file=sys.stderr)
        sys.exit(1)
    ...
    for (name, repository), image_list in image_groups.items():
        ...
        if newer_versions['patch'] or newer_versions['minor']:
            ...
            if newer_versions['patch']: print(f"  Patch updates: {', '.join(newer_versions['patch'])}")
            if newer_versions['minor']: print(f"  Minor/Major updates: {', '.join(newer_versions['minor'])}")
    ...
    if all_new_versions:
        ...
        print("\nSummary of changes:")
        for update in updates:
            print(f"  - {update['image_name']}: {update['old_tag'] or 'N/A'} -> {update['new_tag']} ({update['update_type']})")
    else:
