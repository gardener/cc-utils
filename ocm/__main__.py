...
def add_cref(parsed):
    raw = _parse_yaml_or_json(parsed.file)
    component = raw['component']
    component_references: list = component.get('componentReferences', [])

    name = parsed.name or parsed.component_name.split('/')[-1]
    version = parsed.version
    if not version:
        parts = parsed.component_name.split(':')
        if len(parts) != 2:
            print(f'expected <component-name>:<version>, but saw {parsed.component_name}')
            exit(1)
        version = parts[-1]
    ...
    component_ref = ocm.ComponentReference(
        name=name,
        componentName=parsed.component_name.split(':')[0],
        version=version,
        labels=labels,
    )
