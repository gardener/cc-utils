...
class UpgradePullRequest:
    ...
    def is_obsolete(
        ...
        reference_refs = sorted(
            [
                rc
                for rc in reference_component.componentReferences
...
def iter_obsolete_upgrade_pull_requests(
    ...
    for upgrade_pull_request_group in grouped_upgrade_pull_requests.values():
        ...
        greatest_version = version.parse_to_semver(ordered_by_version[-1].upgrade_vector.whither.version)
...
def split_into_chunks_if_too_long(
    ...
    chunks = tuple(string[start : start + max_chunk_leng] for start in range(0, len(string), max_chunk_leng))
